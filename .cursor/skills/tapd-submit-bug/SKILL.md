...
description: >-
  提交 TAPD 缺陷（bug）并上传截图、日志或录屏附件；创建完成后自动搜索并关联需求（story）；
  支持 dry-run 草稿预览、前端/后端模块路由、简单问题标签、标题格式化与附件失败幂等恢复。
  当用户说要提 bug、提单、报 bug、报缺陷、登记缺陷、录缺陷、提交缺陷、创建 TAPD 工单、把问题提交/录入 TAPD、截图提单、带图提 bug、上传 bug 截图或日志、帮我提交这个问题、这个问题记到 TAPD、预览缺陷单/草稿、确认提交、重试附件上传，或附带问题描述/复现步骤/截图要求写入 TAPD 时使用。
  需配置 .tapd/tapd_env.json（认证）与 project_config.json（项目）。
---
...
以下表述（含同义、口语变体）应优先加载本 Skill，而非泛用 `tapd-plus`：

| 类别 | 示例说法 |
|------|----------|
| 提单动作 | 提 bug、提单、报 bug、报缺陷、登记缺陷、录缺陷、提交缺陷、创建工单、记到 TAPD、录入 TAPD、提交到 TAPD |
| 带附件 | 截图提单、带图提 bug、上传截图、传日志、附录屏、把这张图提上去 |
| 问题描述 | 这个问题帮我提一下、帮我提交这个问题、根据截图提 bug、按这个描述建缺陷单 |
| 草稿确认 | 预览草稿、dry-run、先看看缺陷单、确认后再提交、确认提交 |
| 恢复重试 | 附件上传失败重试、只重传附件、resume-only、别重复建单 |
| 字段相关 | 前端 bug、后端 bug、简单问题、填模块、指定负责人/提单人 |

**典型输入形态**（均应触发）：

- 文字：`登录页白屏，帮我提个 bug`
- 截图 + 简述：`[图] 按钮点不了，提单`
- 结构化：标题 + 复现步骤 + 模块（前端/后端）
- 续操：`确认提交` / `模块选前端`（承接上一轮草稿）
...
1. **（有截图时）** `POST files/upload_image` 上传图片，将返回的 `<img>` 插入 `description` 的 **「实际结果」** 段。
2. 调用 `POST bugs` 创建缺陷（description 含内嵌图片 HTML）。
3. **（可选）** 显式指定 `--attach` 时，调用 `upload-attachment` 上传到附件区。
4. **自动搜索关联需求**（默认开启，见下文「关联需求」）。
5. 返回 `bug_id`、`bug_url`、`embed_images`、`story_link` 及附件信息。

### 截图内嵌（方案 A，默认）

| 文件类型 | 默认行为 |
|----------|----------|
| png/gif/jpg/jpeg/bmp | `upload_image` → 内嵌到「实际结果」 |
| **任意文件 → 附件区** | **默认不上传**；需 `--attach` 或 `attachments.enabled=true` |
| `--no-embed-image` | 图片不内嵌；配合 `--attach` 才进附件区 |
| `--also-attach-image` | 图片内嵌 + 同时挂附件（无需 `--attach`） |

配置示例：

```json
"embed_images": { "enabled": true, "also_attach": false },
"attachments": { "enabled": false }
```

### 关联需求（创建后自动执行）

提交流程在 bug 创建（及附件上传）完成后，**自动调用** `tapd-link-bug-story` 逻辑：
...
python ".cursor/skills/tapd-link-bug-story/scripts/link_bug_story.py" `
  --bug-id <bug_id> --story-id <story_id>
```

草稿 `--dry-run` 时同步预览 `story_link`（含候选需求与置信度）。

详细匹配规则见 Skill **`tapd-link-bug-story`**。
