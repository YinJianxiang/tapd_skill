...
依赖：urllib.request, json, os, base64, argparse, uuid, mimetypes（上传附件）。
环境变量：TAPD_ACCESS_TOKEN 或 TAPD_API_USER + TAPD_API_PASSWORD；
         亦可填写 .tapd/tapd_env.json（access_token 等，脚本自动加载）；
         TAPD_API_BASE_URL（可选，默认 https://api.tapd.cn）；可选 TAPD_BASE_URL（默认 https://www.tapd.cn）, BOT_URL, CURRENT_USER_NICK。
Windows：若仅用 setx 写入用户环境变量且当前进程未继承，会在读取认证前尝试注册表 HKCU\\Environment 与 .tapd/tapd_env.json。
...
命令行用法（AI 可直接调用）：
    ...
    python tapd_client_stdlib.py upload-attachment --workspace-id 123 --entry-id 1169725253001000005 --type bug --file ./screenshot.png
    python tapd_client_stdlib.py upload-image --workspace-id 123 --file ./screenshot.png

Python 调用示例：
    from tapd_client_stdlib import request, get_stories, get_workspace_info, upload_attachment, upload_image
...
# .tapd/tapd_env.json 字段 -> 环境变量名（仅当环境变量未设置时写入）
_TAPD_ENV_FILE_MAP = {
...
def default_tapd_env_path() -> Path:
    return workspace_root() / ".tapd" / "tapd_env.json"
...
def load_local_tapd_env(env_path: Optional[Path] = None) -> None:
    """从 .tapd/tapd_env.json 补全尚未设置的环境变量（一次性本地配置，无需 setx）。"""
    global _local_tapd_env_loaded
    if _local_tapd_env_loaded:
        return
    _local_tapd_env_loaded = True
    path = (env_path or default_tapd_env_path()).resolve()
    if not path.is_file():
        return
    try:
        ...
        return
    for json_key, env_key in _TAPD_ENV_FILE_MAP.items():
...
def load_project_config(config_path: Optional[Path] = None) -> Dict[str, Any]:
    """读取 .tapd/project_config.json；不存在或无效时返回空 dict。"""
    path = (config_path or default_project_config_path()).resolve()
    if not path.is_file():
        return {}
    try:
...
def _maybe_load_token_from_windows_user_env() -> None:
    """setx 写入的用户级变量：已启动的 Cursor/子进程可能未继承，从注册表补足。"""
    if os.environ.get("TAPD_ACCESS_TOKEN"):
        return
    ...
    try:
        ...
        try:
            v, _ = winreg.QueryValueEx(k, "TAPD_ACCESS_TOKEN")
            s = str(v).strip()
            if s:
                os.environ["TAPD_ACCESS_TOKEN"] = s
        finally:
            k.Close()
    except OSError:
        pass
...
def _auth_headers(content_type_json: bool = False) -> dict:
    load_local_tapd_env()
    _maybe_load_token_from_windows_user_env()
...
def _get_headers() -> dict:
    return _auth_headers(content_type_json=True)
...
def _build_multipart(
    ...
    boundary = f"----tapdlib{uuid.uuid4().hex}"
    crnl = b"\r\n"
    ...
    file_ct = guessed or "application/octet-stream"
    ...
    for key, val in fields.items():
        if val is None:
            continue
        chunks.append(b_boundary + crnl)
        chunks.append(f'Content-Disposition: form-data; name="{key}"'.encode("utf-8") + crnl + crnl)
        chunks.append(str(val).encode("utf-8") + crnl)
    ...
    disp = (
        f'Content-Disposition: form-data; name="{file_field}"; filename="{safe_fn}"'
    ).encode("utf-8")
    chunks.append(disp + crnl)
    chunks.append(f"Content-Type: {file_ct}".encode("ascii") + crnl + crnl)
    chunks.append(file_bytes + crnl)
    chunks.append(f"--{boundary}--".encode("ascii") + crnl)
...
def upload_attachment(
    ...
    """
    上传附件并关联到需求/缺陷/任务。对应开放平台 files/upload_attachment。

    :param entry_type: bug、story、task（与官方文档一致，非 stories/tasks）。
    :param entry_id: 工作项 id，一般为长 id（与 GET attachments 的 entry_id 一致）。
    """
    et = entry_type.strip().lower()
    if et not in ("bug", "story", "task"):
        raise ValueError("entry_type 须为 bug、story 或 task")
    ...
    body, ct = _build_multipart(fields, "file", file_path)
    base = _get_base_url()
...
def upload_image(
    ...
    """
    上传图片到 TAPD 平台存储，用于 description 内嵌 <img>。
    对应开放平台 files/upload_image；单张 ≤5MB，后缀 png/gif/jpg/jpeg/bmp。
    返回 data.image_src、data.html_code。
    """
    path = os.path.abspath(file_path)
    if not os.path.isfile(path):
        raise FileNotFoundError(f"图片文件不存在: {path}")
    suffix = Path(path).suffix.lower()
    allowed = {".png", ".gif", ".jpg", ".jpeg", ".bmp"}
    if suffix not in allowed:
        raise ValueError(f"upload_image 仅支持 {', '.join(sorted(allowed))}，当前: {suffix}")
    size = os.path.getsize(path)
    if size > max_bytes:
        raise ValueError(f"图片超过 5MB 限制: {path} ({size} bytes)")

    fields: dict = {"workspace_id": workspace_id}
    body, ct = _build_multipart(fields, "image", file_path)
    base = _get_base_url()
    ...
    if result.get("status") != 1:
        raise RuntimeError(f"upload_image 失败: {json.dumps(result, ensure_ascii=False)}")
    return result
...
def _cli():
    ...
    p_ua = sub.add_parser(
        "upload-attachment",
        help="multipart 上传附件并关联 bug/story/task（开放平台 files/upload_attachment）",
    )
    p_ua.add_argument("--workspace-id", type=int, required=True, dest="workspace_id")
    p_ua.add_argument("--entry-id", required=True, help="工作项 id，一般为长 id")
    p_ua.add_argument(
        "--type",
        required=True,
        choices=("bug", "story", "task"),
        dest="entry_type",
        help="业务类型：bug / story / task",
    )
    p_ua.add_argument("--file", required=True, dest="file_path", metavar="PATH", help="本地文件路径")
    p_ua.add_argument("--owner", help="上传者昵称（可选）")

    p_ui = sub.add_parser(
        "upload-image",
        help="multipart 上传图片到 TAPD（files/upload_image，用于 description 内嵌）",
    )
    p_ui.add_argument("--workspace-id", type=int, required=True, dest="workspace_id")
    p_ui.add_argument("--file", required=True, dest="file_path", metavar="PATH", help="本地图片路径")
    ...
    if args.command == "projects":
        ...
        out = upload_image(args.workspace_id, args.file_path)
