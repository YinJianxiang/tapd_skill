---
name: tapd-plus
description: 查询、创建或更新 TAPD 需求、任务、缺陷、评论、工作流、迭代、测试用例、Wiki、工时、发布计划，或发送企业微信通知。使用 Python 标准库调用 TAPD 开放 API。当用户提到 TAPD API、需求、缺陷、迭代、工时等时使用。
---
...
- 获取需求/任务/缺陷中的图片下载链接或附件信息；**multipart 上传附件**并关联缺陷/需求/任务（开放平台 `files/upload_attachment`）
...
**用户无需手动开 CMD。** Agent 在对话中直接执行脚本；配置从 `.tapd/tapd_env.json` 自动加载（环境变量可覆盖）。

| 配置来源 | 说明 |
|----------|------|
| `.tapd/tapd_env.json` | 推荐：`access_token` 或 `api_user` + `api_password` |
| 环境变量 | `TAPD_ACCESS_TOKEN` 等；**优先级高于本地文件** |
...
`tapd_env.json` 字段：`access_token`、`api_user`、`api_password`、`api_base_url`、`base_url`、`current_user_nick`。

Windows：若仍使用 `setx` 写入用户环境变量，已启动的进程可能未继承；脚本还会在读取认证前尝试注册表 `HKCU\Environment` 与 `.tapd/tapd_env.json`。
...
- **Headers**：
  ...
  - **`Content-Type: application/json`**（常规 JSON POST；附件上传除外）
  - `Via: mcp`
- **Body**：常规 POST 使用 JSON；GET 参数放在 query string。**附件上传** `POST files/upload_attachment` 使用 **`multipart/form-data`**，字段见 [reference/api_reference.md](./reference/api_reference.md)。
...
涉及 id 转换的常见场景：stories/tasks 的 id、bugs 的 id、comments 的 entry_id、`files/upload_attachment` 的 **`entry_id`（一般用工作项长 id）**、`get_scm_copy_keywords` 的 object_id 等。
...
| 上传附件 | POST | files/upload_attachment | **multipart**：`workspace_id`、`entry_id`、`type`=`bug`|`story`|`task`、`file`(文件字段名)；可选 `owner`。先于工作项创建再上传；单次一个文件。 |
...
1. `POST bugs` 创建缺陷，记下返回中的缺陷 **长 id**。  
2. `POST files/upload_attachment`（multipart）：`workspace_id`、`entry_id`=该长 id、`type`=`bug`、表单文件字段 **`file`**。也可用脚本的 **`upload-attachment`** 子命令。
...
# 上传附件到缺陷/需求/任务（multipart；--type 为 bug/story/task，entry_id 多为长 id）
python3 {baseDir}/scripts/tapd_client_stdlib.py upload-attachment --workspace-id <项目ID> --entry-id <工作项长ID> --type bug --file /path/to/screenshot.png [--owner 昵称]
...
**说明**：附件上传已用 **`upload-attachment`** 子命令封装（勿用 `post` 传 JSON）。其余未列出的能力（如评论、工作流、Wiki、工时、企业微信等）可通过 `get` / `post` 配合 [reference/api_reference.md](./reference/api_reference.md) 自行拼装调用。
...
- **推荐**：在 `.tapd/tapd_env.json` 填写 `access_token`（一次性，勿提交仓库）；或用环境变量 `TAPD_ACCESS_TOKEN`。
- 亦可用 `api_user` + `api_password` 或 `TAPD_API_USER` + `TAPD_API_PASSWORD`。
...
- 附件上传接口的 **`type`** 为 `bug` / `story` / `task`；与 **`GET attachments`** 查询参数里的 **`type`（常为 story/bug）** 命名习惯不同，勿混用。
