...
| POST | files/upload_image | multipart：`workspace_id`、`image`(文件字段) | **上传图片供 description 内嵌**；返回 `image_src`、`html_code`；单张 ≤5MB；后缀 png/gif/jpg/jpeg/bmp。 |
| POST | files/upload_attachment | multipart：`workspace_id`、`entry_id`、`type`、`file`；可选 `owner` | **上传附件并关联工作项**。`Content-Type: multipart/form-data`。`type` 取 **`bug` / `story` / `task`**（与官方一致，勿写 stories/tasks）。`entry_id` **一般为工作项长 id**。单次仅一个文件；单文件大小以开放平台说明为准（云环境常见上限约 150MB）。URL 仍需带 `s=mcp`。认证与其它接口相同（Bearer 或 Basic），**勿**再设整包 `Content-Type: application/json`。私有化若 Base 不同，仍以 `TAPD_API_BASE_URL/files/upload_attachment` 为准。 |
| GET | attachments | workspace_id, entry_id, type(story/bug) | 附件列表含 download_url。 |
| GET | attachments/down | workspace_id, id(附件 id) | 附件下载。 |

开放平台另有 `type=story_custom_field` 等变种（需 `custom_field` 等额外参数），见官方说明；本仓库脚本 `upload-attachment` / `upload_attachment()` **仅封装** `bug` / `story` / `task` 三类。
