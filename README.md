...
基于 TAPD 开放 API 的自动化工具集，供 Cursor / Codebuddy Agent 在对话中完成缺陷提单、关单、统计、关联需求等操作。脚本仅依赖 Python 标准库，无需额外安装依赖。
...
配置完成后，在 Cursor 对话里直接描述需求即可，Agent 会自动加载对应 Skill 并执行脚本，**无需手动开 CMD**。
...
- Python 3.8+
- 已安装 [Cursor](https://cursor.com)（或 Codebuddy）
- 本仓库已放到工作区，或作为 Cursor Skill 接入目标项目
